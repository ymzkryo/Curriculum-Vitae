...
クライアントが非IT系だったので、追加機能の要求分析フェーズから担当。  
なるべく専門用語を使わずに要件定義を行い、各種設計書は外部ベンダーと共にレビューを  
行った。  
システムの仕様上、古い取引データが無限に増え続けるため、過去1年間の取引データのみを  
DBに保存し、古い取引データは別データベースで管理し、既存システムとは別のシステムで  
参照できるようにした。  
...
PHPのバージョンを5系から7系に移行する際に、全体のリファクタリングを行った。  
前任者が同一処理を複数箇所に記述した部分を関数化し、コード量を20%ほど削減した。  
併せてオンプレサーバからAWSへの移行を進めた。  
...
監視した結果をslackに通知するためにPerlで簡単なスクリプトを書いた。  
...
世界中のどこからアクセスされても2秒以内にレスポンスを返す必要があったため、  
キャッシュサーバ(CloudFront)を前段に配置して、レスポンスを担保した。  
生産統計データはSQLの生ファイルで5分間に1回以上のペースで生成される仕様だったので  
設計段階ではLambdaで並列処理によるDBへの流し込みを考えていたが、  
LambdaのタイムアウトとDBへの流し込み時間が合わなかったため、サーバでのポーリング処理に  
置き換えた。  
